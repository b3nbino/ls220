...
/**
 * 1. Do a binary search while left <= right
 *    - If the elements around mid are smaller than mid then mid is a peak, return mid
 *    - Else if the left side is greater look left
 *    - Else look right
 */
...
function findPeakInTerrain(terrain) {
  ...
  while (left <= right) {
    ...
    if (terrain[mid] > leftSide && terrain[mid] > rightSide) {
      return mid;
    } else if (terrain[mid] < leftSide) {
      right = mid - 1;
    } else {
      left = mid + 1;
    }
