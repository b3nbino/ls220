...
function nexusSurge(actions) {
  ...
  actions.forEach((action) => {
    if (action === "+") {
      let last = scoreboard[scoreboard.length - 1];
      let penultimate = scoreboard[scoreboard.length - 2];
      scoreboard.push(last + penultimate);
    } else if (action === "*") {
      let last = scoreboard[scoreboard.length - 1];
      scoreboard.push(last * 2);
    } else if (action === "-") {
      scoreboard.pop();
    } else {
      let score = parseInt(action);
